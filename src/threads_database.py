...
def get_threads_posts_by_blog_post_id(blog_post_id: int) -> list:
    conn = get_connection()
    rows = conn.run(
        f"SELECT * FROM threads_posts WHERE source_blog_post_id = {int(blog_post_id)} ORDER BY created_at DESC"
    )
    return _rows_to_dicts(conn, rows)
...
def create_threads_post(content: str, market: str = None, scheduled_at=None,
    ...
    rows = conn.run(
        """
        INSERT INTO threads_posts (content, market, brand, status, scheduled_at, editor_email, source_blog_post_id, source)
...
def get_threads_posts_scheduled_from(start_date: str) -> list[dict]:
    """Fetch posts with scheduled_at >= start_date, ordered ascending."""
    conn = get_connection()
    rows = conn.run(
        "SELECT * FROM threads_posts WHERE scheduled_at >= :start ORDER BY scheduled_at ASC",
        start=start_date,
    )
    return _rows_to_dicts(conn, rows)


def get_threads_posts_posted_in_range(start_date: str, end_date: str) -> list[dict]:
    """Fetch posted posts within a posted_at range (for style reference)."""
    conn = get_connection()
    rows = conn.run(
        "SELECT * FROM threads_posts WHERE status = 'posted' AND posted_at >= :start AND posted_at < :end ORDER BY posted_at ASC",
        start=start_date,
        end=end_date,
    )
    return _rows_to_dicts(conn, rows)
...
def delete_threads_posts_scheduled_from(start_date: str) -> int:
    """Delete posts scheduled from start_date onward and their audit log entries. Returns count deleted."""
    conn = get_connection()
    id_rows = conn.run(
        "SELECT id FROM threads_posts WHERE scheduled_at >= :start",
        start=start_date,
    )
    ...
    conn.run(f"DELETE FROM threads_audit_log WHERE post_id IN ({id_list})")
    conn.run("DELETE FROM threads_posts WHERE scheduled_at >= :start", start=start_date)
    return len(ids)
